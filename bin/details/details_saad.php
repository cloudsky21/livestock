...
if(!isset($_SESSION['token'])) { header("location: ../logmeOut");}
	...
	foreach ($result as $row){	
		...
		$program = $row['program'];
		$group = $row['groupName'];
		$lsp = $row['lsp'].sprintf("%04d",$ids)."&nbsp;SAAD";
...
<table class="table table-condensed table-sm">
	<tbody>
		...
		<tr>
			...
			<td><?PHP if(file_exists('../uploads/SAAD/'.$_SESSION['insurance'].'/'.$ids.'SAAD.pdf')){ echo '<a href="../uploads/SAAD/'.$_SESSION['insurance'].'/'.$ids.'SAAD.pdf" download>'.$ids.'.pdf</a><br>';} else { echo "<strong>No File Uploaded</strong>";} ?></td>
		</tr>
	</tbody>
</table>