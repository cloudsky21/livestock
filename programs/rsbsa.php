...
if(!isset($_SESSION['isLogin']) || (!isset($_COOKIE["lx"]))) { header("location: logmeOut");}
	...
	if(isset($_POST['submiter'])){
		...
		$getdate = $rsbsa_stuff->getDate(date("Y-m-d", strtotime($_POST['rcv'])));

		$ids = "RO8-".date("Y")."-".date("m");

		$LSP = $rsbsa_stuff->lsp($_POST['animal-type']);
		...
		$office_assignment = $rsbsa_stuff->office_assignment($province,$town);
		...
		$result = $rsbsa_stuff->insertData($db,$values,$table);
		if($result>0){
			...
			<div class="table-responsive">
				<div style="overflow-x:auto;">
					...
					//echo '<td><a class="btn btn-xs" role="button" href="#" data-toggle="modal" data-target="#aF" data-id="'.$row['idsnumber'].'" data-backdrop="static"><span class="glyphicon glyphicon-plus" ></span></a></td>';
											...
											echo '<td><a class="btn btn-outline-primary btn-sm" href="../processingslip?ids='.$row['idsnumber'].'PPPP" target="_blank"><i class="fa fa-newspaper-o"> </i></a></td>';
