...
<select name="address" onchange="location = this.value;" class="form-control">
	...
	<option value="biliran.php">Biliran</option>
	<option value="ws.php">Western Samar</option>
	<option value="ns.php" selected>Northern Samar</option>
...
$rs3 = $db->prepare("SELECT COUNT(idsnumber) AS total FROM $table WHERE address LIKE ?");
$rs3->execute(['%N.S%']);
foreach($rs3 as $row){
$getcount = $row['total'];
}
...
else echo "<li class='previous' style='cursor:pointer'><a href='ns.php?page=".($page-1)."' class='pager'>Previous</a></li>";

if($page < $total_pages){
echo "<li class='next' style='cursor:pointer'><a href='ns.php?page=".($page+1)."' class='pager'>Next</a></li>";
}
else {};
echo '</ul>';
...
<?PHP
	...
	$rs = $db->prepare("SELECT * FROM $table WHERE address LIKE ? ORDER BY idsnumber DESC LIMIT ?, ?");
	$rs->execute(["%N.S%",$start_from, $results_per_page]);
